 ...
 */
API.getCats = keyword => {
  return fetch(`${API_ENDPOINT}/cats/search?q=${keyword}`).then(res => res.json());
};
/**
 ...
 */
API.getCat = id => {
  return fetch(`${API_ENDPOINT}/cats/${id}`).then(res => res.json());
};
/**
 ...
 */
API.random50 = _ => {
  return fetch(`${API_ENDPOINT}/cats/random50`).then(res => res.json());
};
